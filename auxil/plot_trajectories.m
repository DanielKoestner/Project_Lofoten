function plot_trajectories(Data, color, title1, fn_png, float_ids, ...
    lines, lgnd, sz, mark_estim, sensor, maplimits,txt,figshow,showgridded)
...
%
%   maplimits: map boundaries of figures: if set to 'none', determined by profile(s) lat/lons
%
%  txt: optional text to add to each profile in amp
%
%  figshow: option to turn off figure display in matlab -- for situations you want to save the figure but not show it
%  showgridded: option to show gridded surface product underlying float trajectory. Default is 'none'
% OUTPUT: None
%
% AUTHORS:
...
% Modified: Aug 19, 2023: Auth Paul Lerner: Added option to set maplimits, add text to profiles, and not show figure in display
...
% Set lat and lon limits
  ...
  latlim = [maplimits.lat_lim(1)-Settings.pad_lat, maplimits.lat_lim(2)+Settings.pad_lat];
  lonlim = [maplimits.lon_lim(1)-Settings.pad_lon, maplimits.lon_lim(2)+Settings.pad_lon];
end
...
if strcmp(Settings.mapping, 'native')
    ...
    for i = 1:nfloats
        ...
        elseif strcmp(color, 'mode')
            ...
            if strcmp(txt,'none') ~= 1;
              text(Data.(floats{i}).LATITUDE(1,idxk{i}), ...
                lon{i}(idxk{i}),txt);
            end
        end
    end
...
elseif strcmp(Settings.mapping, 'm_map') % use "m_map" if indicated
    ...
    hold on;
    % Plot float trajectories on map (points)
    if strcmp(showgridded,'none') == 0;
% create contour plot of gridded dataset
      lat2 = showgridded.lat; lon2 = showgridded.lon;
      [LAT,LON] = meshgrid(lat2,lon2);
      LAT2 = LAT(:);
      LON2 = LON(:);
      LAT2 = LAT2(find(LON<=lonlim(2) & LON>=lonlim(1) & LAT<=latlim(2) & LAT>=latlim(1)));
      LON2 = LON2(find(LON<=lonlim(2) & LON>=lonlim(1) & LAT<=latlim(2) & LAT>=latlim(1)));
      var2 = showgridded.var(:);
      var2 = var2(find(LON<=lonlim(2) & LON>=lonlim(1) & LAT<=latlim(2) & LAT>=latlim(1)));
      LAT3 = reshape(LAT2,[length(find(lon2>=lonlim(1) & lon2<=lonlim(2))),length(find(lat2>=latlim(1) & lat2<=latlim(2)))]);
      LON3 = reshape(LON2,[length(find(lon2>=lonlim(1) & lon2<=lonlim(2))),length(find(lat2>=latlim(1) & lat2<=latlim(2)))]);
      var3 = reshape(var2,[length(find(lon2>=lonlim(1) & lon2<=lonlim(2))),length(find(lat2>=latlim(1) & lat2<=latlim(2)))]);
      contour(LON3,LAT3,var3,20,'LineWidth',0.5,'HandleVisibility','off');h=colorbar;title(h, 'ADT (m)');hold on; % absolute dynamic topography
    end
    for i=1:nfloats
        if strcmp(color, 'multiple') && nfloats > 1
            cidx = round(1 + (ncolor-1) * (i-1) / (nfloats - 1));
            ...
            if strcmp(txt,'none') ~= 1;
              text(lon{i}, Data.(floats{i}).LATITUDE(1,:),txt);
            end
        else
            scatter(lon{i}, Data.(floats{i}).LATITUDE(1,:), ...
