...
%                   points (default: 36)
%
%   maplimits: map boundaries of figures: if set to 'none', determined by profile(s) lat/lons. default is 'none'
%
%   txt: optional text to add to each profile in map. default is 'none' (no text)
%
%   figshow: option to turn off figure display in matlab -- for situations you want to save the figure but not show it. Default is 'on'
%   showgridded: option to show a gridded surface product underlaying float trajectories. Default is set to 'none'
% OUTPUT:
...
% Modified by Paul Lerner Aug 19, 2023: Added options for map lat/lon limits, showing text for profiles, and not displaying figure
global Settings;
...
for i = 1:2:length(varargin)-1
    ...
    elseif strcmpi(varargin{i}, 'sensor')
        sensor = varargin{i+1};
    elseif strcmpi(varargin{i}, 'maplimits')
        maplimits = varargin{i+1};
    elseif strcmpi(varargin{i}, 'txt')
        txt = varargin{i+1};
    elseif strcmpi(varargin{i}, 'figshow')
        figshow = varargin{i+1};
    elseif strcmpi(varargin{i}, 'showgridded');
        showgridded = varargin{i+1};
    else
...
if isempty(good_float_ids)
    ...
    plot_trajectories(Data, color, title, fn_png, good_float_ids, lines, ...
        lgnd, sz, mark_estim, sensor,maplimits,txt,figshow,showgridded);
end
